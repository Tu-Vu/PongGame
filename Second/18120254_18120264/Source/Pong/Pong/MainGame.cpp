...
void MainGame::Initialize(RenderWindow* window) {
	...
	for (int i = 0; i < 6; i++) { 
		int type; // loại element
		type = 1 + rand() % (4 + 1 - 1);  // random só từ 1 đến 4
		this->ElementObject[i] = new Element(this->Score1,this->Score2,this->BallObject, type); // tạo loại element ngẫu nhiên
		if (i == 0)
			this->ElementObject[i]->setPosition(window->getSize().x / 2 + 100, window->getSize().y / 2 + 200); // đặt vị trí cho element 
		else if (i == 1)
			this->ElementObject[i]->setPosition(window->getSize().x / 2, window->getSize().y / 2 + 200); // đặt vị trí cho element 
		else if (i == 2)
			this->ElementObject[i]->setPosition(window->getSize().x / 2 - 100, window->getSize().y / 2 + 200); // đặt vị trí cho element 
		else if (i == 3)
			this->ElementObject[i]->setPosition(window->getSize().x / 2, window->getSize().y / 2 - 200); // đặt vị trí cho element 
...
void MainGame::Update(RenderWindow* window) {
	...
	for (int i = 0; i < 6; i++) {
		this->ElementObject[i]->Update(); // update sự kiện cho element
	}
...
void MainGame::Destroy(RenderWindow* window) { // hàm hủy
	delete this->Player1;
	delete this->Player2;
