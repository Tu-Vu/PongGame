...
WaterMapAI::WaterMapAI(int Score1, int Score2) {
	this->prevalue1 = Score1;  // lưu lại điểm người chơi từ element state
	this->prevalue2 = Score2;
}
void WaterMapAI::Initialize(RenderWindow* window) {
...
void WaterMapAI::Update(RenderWindow* window) {
...
void WaterMapAI::Render(RenderWindow* window) { // vẽ các object có trong trò chơi
...
void WaterMapAI::Destroy(RenderWindow* window) {
	delete this->Player1;
	delete this->Player2;
